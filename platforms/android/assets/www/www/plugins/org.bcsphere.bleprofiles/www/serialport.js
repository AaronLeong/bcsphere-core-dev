...
var serialport = {
	...
	write : function(device,writeType,writeValue,successFunc,errorFunc){
		if(API == "ios" && serialport.role == serialport.SLAVE){
			service.getCharacteristicByUUID(serialport.readcharUUID)[0].notify(writeType,writeValue,successFunc,errorFunc);
		}else{
			if(device.type == "Classical"){
	...
	subscribe : function(device,callback){
		if(device.type == "Classical"){
			...
			if(serialport.role == "Master"){
				...
				var writecharacter = service.getCharacteristicByUUID(serialport.writecharUUID)[0];
				writecharacter.addEventListener("oncharacteristicwrite",callback);
			}
		}
	},
	...
	unsubscribe : function(device){
		if(device.type == "Classical"){
			...
			if(serialport.role == "Master"){
				...
				writecharacter.addEventListener("oncharacteristicwrite",null);
			}
		}
	}, 
